...
export async function open({ url, left, top, width, height, modal, opener } = {}, dialogContentsParams = {}) {
	const lastId = (await browser.storage.session.get("dialog:lastWin"))["dialog:lastWin"];
	if (lastId) {
		mLogger(`window ${lastId} is already open`);
		return;
	}
	const id = generateId();
				...
				async () => {
					...
					const [readyEvent,] = await Promise.all([
						new Promise(resolveReady => {
							loader.contentDocument.addEventListener(TYPE_READY, event => {
								mLogger(`onReady at ${id}`);
							...
							if ('interactive' == loader.contentDocument.readyState || 'complete' == loader.contentDocument.readyState) {
								notifyReady(loader.contentWindow);
							} else {
								loader.contentWindow.addEventListener('DOMContentLoaded', () => notifyReady(loader.contentWindow));
							}
						}),
	...
	async function safeCreateWindow(params) {
		const existingWindowIds = new Set((await browser.windows.getAll()).map(win => win.id));
		const uniqueKeyParam = `popup-id-for-${id}=${generateId()}`;
		...
		const promisedWin = browser.windows.create({
			...
			mLogger('safeCreateWindow: promised window is resolved');
			win = resolvedWin;
		});
		while (!win) {
			await Promise.race([
				new Promise(async (resolve, _reject) => {
					...
					for (const window of windows) {
						...
						mLogger('safeCreateWindow: new window is detected');
						win = window;
						resolve();
						return;
					}
		...
		if (modal || dialogContentsParams.modal) {
			await browser.storage.session.set({ "dialog:lastWin": win.id });
		}
		return win;
	}
	...
	return new Promise(async (resolve, reject) => {
		...
		const onRemoved = windowId => {
			if (!win || windowId != win.id)
				return;
			browser.storage.session.remove("dialog:lastWin");
...
export async function getWindowId(wnd) {
	const params = new URLSearchParams((wnd || window).location.search);
	const id = params.get('dialog-id');
...
export async function notifyReady(wnd) {
	if (!wnd) {
		wnd = window;
	}
	const params = new URLSearchParams(wnd.location.search);
	const id = params.get('dialog-id');
	const type = params.get('dialog-type');
	...
	const detail = {
		id,
		windowWidthOffset: Math.max(0, wnd.outerWidth - wnd.innerWidth),
		windowHeightOffset: Math.max(0, wnd.outerHeight - wnd.innerHeight)
	};
	...
	if (params.get('dialog-offscreen') != 'true') {
		...
		wnd.document.dispatchEvent(new CustomEvent(TYPE_READY, {
			detail,
			bubbles: true,
			cancelable: false,
			composed: true
	...
	return new Promise(async (resolve, _rejet) => {
		const windowId = await getWindowId(wnd);
		...
		setTimeout(() => {
			wnd.document.documentElement.classList.add('ready');
			resolve();
		}, 0);
	});
}
...
function initDialogListener(id) {
	const onMessage = (message, _sender) => {
		...
		switch (message.type) {
			case TYPE_FOCUS:
				browser.windows.update(mCurrentWindowId, { focused: true });
				window.focus();
				break;
...
export async function accept(detail = null) {
	try {
		...
		console.warn(`[dialog] failed to send ${TYPE_ACCEPT}`, e);
	}
}
...
export async function cancel() {
	try {
		...
		console.warn(`[dialog] failed to send ${TYPE_CANCEL}`, e);
	}
}
...
export function initAcceptButton(button, onCommand) {
	initButton(button, async event => {
		await accept(typeof onCommand == 'function' ? (await onCommand(event)) : null);
	});
}

export function initCancelButton(button) {
	initButton(button, async _ => {
		await cancel();
	});
}
